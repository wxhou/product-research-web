...
6. 市场规模和增长趋势数据`;
  ...
  try {
    ...
    throw new Error('LLM 分析失败，请检查 API 配置');
  }
}
...
// 使用 LLM 生成完整报告
async function generateReportWithLLM(project: Project, results: SearchResult[], llmAnalysis: unknown): Promise<string> {
  const config = getLLMConfig();
  const hasApiKey = !!config.apiKey;
  const sources = [...new Set(results.map(r => r.source))].join(', ') || '未知';
  const normalizedAnalysis = (llmAnalysis && typeof llmAnalysis === 'object'
    ? llmAnalysis
    : {}) as {
      features?: unknown[];
...
| 数据来源 | ${sources} |
| 关键词 | ${project.keywords || project.title} |
| 识别功能数 | ${normalizedAnalysis.features?.length || '待分析'} |
| 识别竞品数 | ${normalizedAnalysis.competitors?.length || '待分析'} |
...
radar
    ...
    ${normalizedAnalysis.competitors?.[0]?.name || '竞品A'}: [80, 70, 85, 60, 75, 70]
    ${normalizedAnalysis.competitors?.[1]?.name || '竞品B'}: [70, 85, 75, 80, 65, 75]
...
mindmap
  root((SWOT分析))
    优势
      ::icon(fa fa-star)
      ${(normalizedAnalysis.swot?.strengths || []).slice(0, 3).map((s: string) => `● ${s}`).join('\n      ') || '● 核心优势1\n      ● 核心优势2\n      ● 核心优势3'}
    劣势
      ::icon(fa fa-warning)
      ${(normalizedAnalysis.swot?.weaknesses || []).slice(0, 3).map((w: string) => `● ${w}`).join('\n      ') || '● 主要劣势1\n      ● 主要劣势2\n      ● 主要劣势3'}
    机会
      ::icon(fa fa-lightbulb)
      ${(normalizedAnalysis.swot?.opportunities || []).slice(0, 3).map((o: string) => `● ${o}`).join('\n      ') || '● 市场机会1\n      ● 市场机会2\n      ● 市场机会3'}
    威胁
      ::icon(fa fa-exclamation-triangle)
      ${(normalizedAnalysis.swot?.threats || []).slice(0, 3).map((t: string) => `● ${t}`).join('\n      ') || '● 外部威胁1\n      ● 外部威胁2\n      ● 外部威胁3'}
\`\`\`
...
quadrantChart
    ...
    quadrant 维持现状
    quadrant 低成本试错
    ${(normalizedAnalysis.opportunities || []).slice(0, 8).map((o: string, i: number) => {
...
- 报告内容详细、专业，字数不少于 3000 字`;
  ...
  try {
    ...
    throw new Error('LLM 报告生成失败，请检查 API 配置');
  }
}
