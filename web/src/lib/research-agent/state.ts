...
export function updateSearchingState(
  ...
  return {
    ...
    progress: 10 + Math.min(newResults.length / RESEARCH_QUALITY_THRESHOLDS.minSearchResults, 1) * 20,
    updatedAt: new Date().toISOString(),
  };
}
...
export function updateExtractingState(
  ...
  const extractionTarget = Math.min(
    RESEARCH_QUALITY_THRESHOLDS.minExtractions,
    Math.max(state.searchResults.length, 1)
  );

  return {
    ...updates,
    extractedContent: extracted,
    progress: 30 + Math.min(extracted.length / extractionTarget, 1) * 20,
    updatedAt: new Date().toISOString(),
  };
}
