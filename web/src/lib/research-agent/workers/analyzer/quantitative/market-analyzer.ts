...
export class MarketDataAnalyzer {
  ...
  async analyzeMarketData(keyword: string): Promise<MarketData> {
    ...
    return {
      ...
      forecastYears,
      dataSource: {
        primary: fetchedData.dataSource || '数据缺失',
        secondary: [],
        lastUpdated: new Date().toISOString(),
      },
      confidenceLevel: fetchedData.confidenceLevel || 'Low',
      marketDrivers,
      marketConstraints,
    };
  }
   ...
   * 识别进入壁垒
   */
  private identifyEntryBarriers(competitors: Array<{ name: string; description?: string; marketShare?: number }>): string[] {
