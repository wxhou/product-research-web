...
/** 可用的数据源列表 */
// 从 DataSourceManager 动态获取可用数据源
function getAvailableSources(): DataSourceType[] {
  const { getDataSourceManager } = require('@/lib/datasources');
  const mgr = getDataSourceManager();
  return mgr.getAllSources()
    .filter((s: { enabled: boolean; type: DataSourceType }) => s.enabled && mgr.services.has(s.type))
    .map((s: { type: DataSourceType }) => s.type);
}

/** 默认启用的数据源从数据库获取 */
export function getDefaultSources(): DataSourceType[] {
  const { getDataSourceManager } = require('@/lib/datasources');
  const mgr = getDataSourceManager();
  return mgr.getEnabledSources();
}

/** 每个数据源的默认超时 (30s per search) */
