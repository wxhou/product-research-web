...
import { getJinaReaderService, type JinaReaderService } from '../../../datasources';
...
function createExtractorAgent(config: Partial<ExtractorConfig> = {}) {
  ...
  const jinaReader = getJinaReaderService();

  return {
    config: finalConfig,
    execute: (state: ResearchState) => executeExtract(state, finalConfig, jinaReader),
  };
}
...
async function executeExtract(
  ...
  try {
    ...
    // 使用 Jina Reader 批量爬取
    console.log(`[Extractor] 使用 Jina Reader 爬取 ${urls.length} 个 URL`);
    const crawlResults = await jinaReader.crawlMultiple(urls);
